...
This post  covered 'feature selection' in Machine Learning. Specifically I touch best fit, forward fit, backward fit, ridge(L2 regularization) & lasso (L1 regularization). The post includes equivalent code in R and Python. 
...
In this part I discussed SVMs, Decision Trees, validation, precision recall, and roc curves
...
You can download this R Markdown file and associated data files from Github at [MachineLearning-RandPython-Part5](https://github.com/tvganesh/MachineLearning-RandPython-Part5)

For this part I have used the data sets from [UCI Machine Learning repository](https://archive.ics.uci.edu/ml/datasets.html)([Communities and Crime](https://archive.ics.uci.edu/ml/datasets/Communities+and+Crime) and [Auto MPG](https://archive.ics.uci.edu/ml/datasets/Auto+MPG))
...
latex varphi_{k} , k=1,2,3,..K is a piecewise cubic polynomialwith continuous derivative upto order 2 at each knot
latex y_{i} = \beta_{0} +\beta_{1}b_{1}(x_{i}) +\beta_{2}b_{2}(x_{i}) + .. +  \beta_{K+3}b_{K+3}(x_{i}) + \epsilon_{i}
For each (latex x_{i},y{i}) the basis functions latex b_{i} are called 'basis' functions
latex b_{1}(x_{i})=x_{i}
latex b_{2}(x_{i})=x_{i}^2
latex b_{3}(x_{i})=x_{i}^3
latex b_{k+3}(x_{i})=(x_{i}-\varphi_{k})^3
where k=1,2,3... K
...
Here a Gradient Boosted ML Model is built with a n.trees=5000, with a learning rate of 0.01
and depth of 4. The feature importance plot also shows that rooms and status are the 2 most important features. The MSE vs the number of trees plateaus around 2000 trees
```{r fig10,cache=TRUE,warning=FALSE,message=FALSE}
library(gbm)
...
Included below is a cross validation error vs  the learning rate. The lowest error is
when learning rate = 0.09
```{r fig11,cache=TRUE}
cvError <- NULL
...
A gradient boost ML model in Python is created below. The Rsquared score is computed
...
the cross validation error is computed as the learning rate is varied. The minimum CV eror
occurs when lr = 0.04
```{python}
import numpy as np
import pandas as pd
...
**Conclusion**
This post covered Splines and Tree based ML models like Bagging, Random Forest and Boosting.
Stay tuned for further updates.

You may also like

1. [Re-introducing cricketr! : An R package to analyze performances of cricketer](https://gigadom.wordpress.com/2016/05/14/re-introducing-cricketr-an-r-package-to-analyze-performances-of-cricketers/)
2. [Designing a Social Web Portal](https://gigadom.wordpress.com/2013/02/14/designing-a-social-web-portal/)
3. [My travels through the realms of Data Science, Machine Learning, Deep Learning and (AI)](https://gigadom.wordpress.com/2017/09/11/my-travels-through-the-realms-of-data-science-machine-learning-deep-learning-and-ai/)
4. [My TEDx talk on the "Internet of Things"](https://gigadom.wordpress.com/2013/06/20/my-tedx-talk-on-the-internet-of-things/)
5. [Singularity](https://gigadom.wordpress.com/2011/06/10/singularity/)
6. [A closer look at "Robot Horse on a Trot" in Android](https://gigadom.wordpress.com/2013/07/08/a-closer-look-at-robot-horse-on-a-canter-in-android/)
